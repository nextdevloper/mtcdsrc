...
public class LocationDto implements Serializable {
	 ...
	 */
	public final void setLocationInf(final MLocation mlocation) {

	    locId = mlocation.getLocId();
	    locKind = mlocation.getLocKind();
	    locNm = mlocation.getLocNm();
	    locOnrId = mlocation.getLocOnrId();
	    postcd = mlocation.getPostcd();
	    address = mlocation.getAddress();
	    recipient = mlocation.getRecipient();
	    tel1 = mlocation.getTel1();
	    note = mlocation.getNote();
	    defaultLocale = mlocation.getDefaultLocale();
		exCnt = mlocation.getExCnt().toString();
	    insAcc = mlocation.getInsAcc();
	    insDate = DateUtil.toDateTimestampString(mlocation.getInsDate());
	    updAcc = mlocation.getUpdAcc();
	    updDate = DateUtil.toDateTimestampString(mlocation.getUpdDate());
	    delFlg = mlocation.getDelFlg();
		recId = mlocation.getRecId().toString();
	}

	/**
	 *  ロケーション情報を破棄します.
	 */
	public final void resetLocationInf() {
	    locId = null;
	    locKind = null;
	    locNm = null;
	    locOnrId = null;
